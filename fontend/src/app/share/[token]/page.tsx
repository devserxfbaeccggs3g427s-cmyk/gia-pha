...
import { ShareLinkServiceError, shareLinkService, type SharedTreeView } from '@/lib/services/share-link-service';
import styles from './share.module.css';

type SharedMember = SharedTreeView['members'][number];
...
export default async function SharedTreePage({ params }: { params: Promise<{ token: string }> }) {
  let data;
  try {
    data = await shareLinkService.getSharedTree((await params).token);
  } catch (error) {
...
function compareMembers(left: SharedMember, right: SharedMember): number {
...
function formatLife(member: SharedMember): string {
