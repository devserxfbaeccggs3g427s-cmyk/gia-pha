...
export async function generateMetadata({ params }: ReportsRouteProps): Promise<Metadata> {
  const t = await getTranslations({ locale: (await params).locale, namespace: 'reportsPage' });
  return { title: t('metaTitle'), description: t('description') };
}

export default async function ReportsRoute({ params }: ReportsRouteProps) {
  return <ReportsPage treeId={(await params).treeId} />;
}
