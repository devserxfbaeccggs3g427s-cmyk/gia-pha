...
import { warmGatewaySession } from '@/lib/auth/bridge-exchange';
...
export async function POST(request: Request): Promise<Response> {
  ...
  const cookie = setBridgeCookie(issued, { secure: process.env.NODE_ENV === 'production' });
...
function normalizeStrength(value: unknown): BridgeAuthStrength {
  if (value === 'REAUTHENTICATED' || value === 'HARDWARE') {
    return value;
  }
  return 'SESSION';
}
