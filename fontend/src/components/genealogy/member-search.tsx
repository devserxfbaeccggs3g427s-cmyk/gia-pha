              ...
              {results.map(({ member, matchedFields }, index) => (
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    {matchedFields.length > 0 && <span className="mt-1.5 flex flex-wrap gap-1">{matchedFields.map((field) => { const key = `matched.${field}` as 'matched.fullName' | 'matched.nickname' | 'matched.occupation' | 'matched.placeOfBirth'; return <span key={field} className="rounded-full bg-primary/8 px-2 py-0.5 text-[10px] font-semibold text-primary">{t(key)}</span>; })}</span>}
                  </span>
...
function normalizeResults(body: unknown): DisplayResult[] {
  ...
  return body.flatMap((item): DisplayResult[] => {
    ...
    if ('member' in item && item.member && typeof item.member === 'object') {
      ...
      return [{ member: result.member, matchedFields: result.matchedFields as SearchableMemberField[] }];
    }
