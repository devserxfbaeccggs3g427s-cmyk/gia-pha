...
- [x] 4. Build and verify the Spring service template
  - [x] 4.1 Revalidate the completed template as a feature-oriented layered structure using Spring MVC, Spring Security, Spring for Apache Kafka, gRPC Java, Resilience4j, and Micrometer/OpenTelemetry; organize each feature into controller, DTO/mapper, service, repository, domain, and integration packages without shared business domain models
  - [x] 4.2 Add validation, idempotency, local audit/outbox, inbox/deduplication, retry/DLQ, replay, and owning-service Saga state-machine primitives in the service layer without an external workflow engine
  - [x] 4.3 Add deadline-bound gRPC and REST inbound defaults, feature-local outbound gRPC clients, health, telemetry, and workload-identity defaults
  - [x] 4.4 Add Flyway schema authority and repository-encapsulated Spring Data JDBC aggregate persistence and jOOQ query/generation support with exact MySQL 8.4 Testcontainers support
  - [x] 4.5 Expand JUnit 5, AssertJ, Mockito, Testcontainers, and ArchUnit tests to enforce inbound-to-service-to-repository direction, prohibit controller-to-repository bypass, isolate transport DTOs, encapsulate persistence frameworks, restrict cross-feature internals, and retain database ownership, cross-service isolation, dependency-cycle, synchronous-call-cycle, and unversioned-event checks
  - [x] 4.6 Publish and version technical-only platform starters, OpenAPI contracts, event Protobuf artifacts, and gRPC Protobuf artifacts independently without generic architecture abstractions
  - **Acceptance:** the revalidated template build and tests pass; a representative feature proves REST, Kafka, and gRPC inbound paths through transactional service orchestration, repository-backed Spring Data JDBC/jOOQ persistence, and feature-local Kafka producer/outbound gRPC integration; duplicate/reordered messages are idempotent; event Protobuf compatibility passes; event and gRPC artifacts publish independently; layer, feature, and cross-service architecture violations fail CI; no service credential can reach another database/topic
