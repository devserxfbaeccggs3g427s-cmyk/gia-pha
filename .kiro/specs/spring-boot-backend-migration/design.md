...
Each Spring service uses hexagonal boundaries:

```text
Inbound Adapter -> Application Port -> Domain -> Outbound Port -> Adapter
```

Domain code remains framework-independent. SQL and transport DTOs do not become shared domain objects. Spring Data JDBC persists aggregates and transactional state; jOOQ implements explicit SQL, read models, and projection queries. Flyway migrations are schema authority, and jOOQ generation is reproducible from the migrated schema. The service baseline includes Spring MVC, Spring Security, Spring for Apache Kafka, gRPC Java, Resilience4j, Micrometer/OpenTelemetry, JUnit 5, AssertJ, Mockito, Testcontainers, and ArchUnit.

A reusable versioned platform starter may provide security, telemetry, outbox/inbox, idempotency, gRPC policy, health checks, and test harnesses, but may not contain shared business models. Saga state machines remain in the owning service; no external workflow engine is included in the baseline. Protobuf event contracts and Protobuf gRPC service contracts have separate generation, versioning, compatibility, and publication lifecycles.
