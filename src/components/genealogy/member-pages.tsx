...
import type { Event, Gender, MediaMetadata, Member, Relationship, RelationType, SourceReference, SourceProvenance } from '@/data/types';
...
export function MemberListPage({ treeId, treeKind = 'STANDALONE', sourceTrees = [] }: { treeId: string; treeKind?: 'STANDALONE' | 'COMPOSITE'; sourceTrees?: Array<{ treeId: string; name: string }> }) {
  ...
  const composite = treeKind === 'COMPOSITE';
  const availableSourceTrees = sourceTrees.length > 0 ? sourceTrees : [...new Set(members.flatMap((member) => 'sourceReferences' in member ? ((member as Member & { sourceReferences?: SourceReference[] }).sourceReferences ?? []).map((reference) => reference.treeId) : []))].map((sourceTreeId) => ({ treeId: sourceTreeId, name: sourceTreeId }));
  const [query, setQuery] = useState(''); const [status, setStatus] = useState<'ALL' | 'ALIVE' | 'DECEASED'>('ALL'); const [gender, setGender] = useState<'ALL' | Gender>('ALL'); const [selectedSourceTreeId, setSelectedSourceTreeId] = useState(sourceTrees[0]?.treeId ?? ''); const [formOpen, setFormOpen] = useState(false); const [avatarSaving, setAvatarSaving] = useState(false); const saving = memberMutation.isPending || avatarSaving;
...
export function MemberDetailPage({ treeId, memberId }: { treeId: string; memberId: string }) {
  ...
  const [detail, setDetail] = useState<Member & { relationships: Relationship[]; relatedMembers: Member[]; events: Event[]; media: MediaMetadata[]; lifespan: number | null; sourceReferences?: SourceReference[]; provenance?: SourceProvenance[]; conflictingFields?: string[]; hasConflictingFields?: boolean }>(); const [members, setMembers] = useState<Member[]>([]); const [loading, setLoading] = useState(true); const [error, setError] = useState(''); const [editOpen, setEditOpen] = useState(false); const [relationOpen, setRelationOpen] = useState(false); const [deleteOpen, setDeleteOpen] = useState(false); const [avatarSaving, setAvatarSaving] = useState(false); const saving = memberMutation.isPending || avatarSaving;
