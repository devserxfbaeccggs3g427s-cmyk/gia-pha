...
export function MemberListPage({ treeId, treeKind = 'STANDALONE' }: { treeId: string; treeKind?: 'STANDALONE' | 'COMPOSITE' }) {
  ...
  const sourceTreeIds = [...new Set(members.flatMap((member) => 'sourceReferences' in member ? ((member as Member & { sourceReferences?: SourceReference[] }).sourceReferences ?? []).map((reference) => reference.treeId) : []))];
