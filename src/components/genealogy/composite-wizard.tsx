...
import type { CompositeTreeConfig, FamilyTree, SourcePreview, SourceReference } from '@/data/types';
...
export function CompositeWizard({ tree, availableTrees }: { tree: FamilyTree; availableTrees: FamilyTree[] }) {
  ...
  return <Card className="border-primary/20"><CardHeader><CardTitle>{t('title')}</CardTitle></CardHeader><CardContent className="space-y-5">
    ...
    {step === 4 && <div className="space-y-3"><div className="grid gap-3 md:grid-cols-2">{[ { value: linkSource, set: setLinkSource, label: t('linkSource') }, { value: linkTarget, set: setLinkTarget, label: t('linkTarget') } ].map(({ value, set, label }) => <div key={label} className="grid gap-2"><label className="text-sm">{label}<select className="mt-1 h-10 w-full rounded-md border bg-background px-3" value={value.treeId} onChange={(event) => set({ ...value, treeId: event.target.value })}><option value="">{t('chooseSource')}</option>{config?.sources.map((source) => <option key={source.id} value={source.sourceTreeId}>{sourceName(source.sourceTreeId)}</option>)}</select></label><Input value={value.memberId} onChange={(event) => set({ ...value, memberId: event.target.value })} placeholder={t('memberId')} /></div>)}</div><Button disabled={!linkSource.treeId || !linkSource.memberId || !linkTarget.treeId || !linkTarget.memberId || busy} onClick={() => void request('relationships', 'POST', { source: linkSource, target: linkTarget, type: 'PARENT_CHILD' })}>{t('addLink')}</Button><div className="space-y-2">{config?.crossTreeRelationships.map((relationship) => <div key={relationship.id} className="flex items-center gap-2 rounded-lg border p-3 text-sm"><span className="flex-1">{relationship.source.memberId} → {relationship.target.memberId}</span><Button size="icon" variant="ghost" onClick={() => void request(`relationships/${relationship.id}`, 'DELETE').then(load)}><Trash2 /></Button></div>)}</div><Button variant="outline" onClick={() => void validate()}>{t('validate')}</Button></div>}
